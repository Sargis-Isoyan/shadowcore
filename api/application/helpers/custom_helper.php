...
if (!function_exists('send_email')) {
    function send_email($options)
    {
        ...
        if($options['mail_template']){
            $CI->email->set_mailtype('html');
            $filePath = UPLOAD_PATH . 'templates/'.$options['mail_template'].'.html';
            $content = file_get_contents($filePath);
            $mailContent = str_replace(array("{{title}}","{{content}}","{{email}}"), array($options['subject'], $options['message'], $options['to']), $content);

            if(sizeof($options['params'])){
                $keys = array();
                $values = array();
